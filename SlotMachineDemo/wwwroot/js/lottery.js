...
$.fn.playSpin = function (options) {
    if (this.length) {
        if ($(this).is(':animated')) return; //如果此元素正在设置动画，则返回false
...
$.fn.stopSpin = function () {
    if (this.length) {
        if (!$(this).is(':animated')) return; //如果此元素未设置动画，则返回false
...
var slotMachine = function (el, options, track) {
    ...
    slot.defaultOptions = {
        easing: 'swing',        // String: 最终旋转的缓动类型
        time: 3000,             // Number: 旋转动画的总时间
        loops: 6,               // Number: 在动画中旋转的时间
        manualStop: false,      // Boolean: 旋转直到用户手动单击停止
        stopSeq: 'random',      // String: 序列结束的动画, random, leftToRight, rightToLeft
        endNum: 0,              // Number: 动画结束于列表的编号/顺序
        onEnd: $.noop,          // Function: 在每个元素旋转结束后运行，将传递参数endNum
        onFinish: $.noop        // Function: 在所有元素旋转结束后运行，将传递参数endNum
    };
    ...
    slot.setup = function () {
        ...
        $li.clone().appendTo(slot.$el); //克隆到最后一行以获得流畅的动画
    ...
    slot.endSpin = function () {
        ...
        slot.$el
            ...
            .animate({'top': finalPos}, finalSpeed, slot.options.easing, function () {
                slot.$el.find('li').last().remove(); // 删除克隆的行
